  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Overview of the entire placement system"
      />

      <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
        {[
          { label: "Total Students", value: data.summary?.total_students },
          { label: "Total Companies", value: data.summary?.total_companies },
          { label: "Total Jobs", value: data.summary?.total_jobs },
          { label: "Total Applications", value: data.summary?.total_applications },
          { label: "Placement Rate", value: `${data.summary?.placement_rate}%` },
        ].map((card, i) => (
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div className="interactive-card">
          <h2 className="text-lg font-bold text-[var(--primary)] mb-4">Students by Faculty</h2>
          <ResponsiveContainer width="100%" height={250}>
            <PieChart>
              <Pie data={data.faculty_chart} dataKey="count" nameKey="faculty" cx="50%" cy="50%" outerRadius={80} label>
                {data.faculty_chart?.map((entry, index) => (
                  <Cell key={index} fill={COLORS[index % COLORS.length]} />
                ))}
              </Pie>
              <Tooltip />
      ...
      <div className="interactive-card">
        <h2 className="text-lg font-bold text-[var(--primary)] mb-4">Company Hiring</h2>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data.company_hiring}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="company" tick={{ fontSize: 12 }} />
            <YAxis tick={{ fontSize: 12 }} />
            <Tooltip />
            <Bar dataKey="selected" fill="#1a2f6e" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>

      <div className="interactive-card">
        <h2 className="text-lg font-bold text-[var(--primary)] mb-4">Applications per Job</h2>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data.jobs_chart}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="job" tick={{ fontSize: 12 }} />
            <YAxis tick={{ fontSize: 12 }} />
            <Tooltip />
            <Bar dataKey="applications" fill="#e85d1e" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
